...
export async function aggregateQuotes(quotes, { axios, context = {} } = {}) {
    ...
    // Resting on one or two live listings rather than a market. Worth showing —
    // a scarce card is not a worthless one — but not worth trusting much, and
    // the number on screen has to say which it is.
    if (finalPool.some(q => q.thin)) confidence -= 0.2;
    confidence = Math.max(0.1, Math.min(1, Number(confidence.toFixed(2))));

    // Three weak signals at once means this number is not about this card.
    //
    // Nothing matched the printing; the printing was never confirmed in the
    // first place; and every quote is a live asking price rather than something
    // that actually sold. Each alone is survivable — a scarce card really may
    // have no completed sales — but together there is nothing tying any of
    // these listings to the card in hand, and the median of a pool that is
    // about some other object is a confident number about that other object.
    //
    // This is how a common Crobat came to be carried at five thousand dollars:
    // the search could not be pinned to the printing, matched a graded lot, and
    // the lot's price became the card's. Trimming cannot catch it, because the
    // whole pool agrees — it is simply agreeing about the wrong thing. Refusing
    // here leaves the card unpriced, which is the truth, and unpriced cards are
    // already surfaced for another look.
    const nothingTiesToThisCard = !variantMatched
        && context.verified === false
        && finalPool.every(q => q.thin || q.basis === 'listings');
    if (nothingTiesToThisCard) return null;
