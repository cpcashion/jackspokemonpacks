...
/**
 * The denominator of a printed card number — "093/132" → 132.
 *
 * This is the strongest set discriminator printed on a card, and discarding it
 * was the reason cards ended up needing human review. Without it, "Steelix
 * 093/132" could only be placed by trusting whatever set name the AI guessed,
 * and a guess of "Temporal Forces" (162 cards) is not merely wrong, it is
 * checkably wrong: no card in a 162-card set is numbered out of 132.
 *
 * Returns 0 when there is no denominator — promos and subsets like "TG12/TG30"
 * and "SWSH045" often have none, so a 0 means "no evidence", never "mismatch".
 */
export function printedSetTotal(value) {
    const raw = String(value || '').normalize('NFKC').trim();
...
/**
 * Everything a printed card number tells us, in one place.
 * @returns {{number: string, printedTotal: number}}
 */
export function parseCardNumber(value) {
    return {
        number: normalizeCardNumber(value),
        printedTotal: printedSetTotal(value),
    };
}
...
/**
 * Rarities that are only ever printed with a foil artwork box.
 *
 * This exists because the foil pattern is the single hardest thing to read off
 * a photograph — it depends on the angle the light happened to hit the card —
 * and getting it wrong is expensive. A Base Set Charizard priced as the
 * non-holo variant of itself is off by two orders of magnitude.
 *
 * The rarity, by contrast, is printed as a symbol and the card databases return
 * it as a string. "Rare Holo" means holo. That is not an inference about the
 * photo; it is what the card says it is.
 */
const ALWAYS_HOLO_RARITY = [
    'rare holo', 'holo rare', 'rare ultra', 'ultra rare', 'rare secret', 'secret rare',
    'illustration rare', 'special illustration rare', 'rare rainbow', 'rainbow rare',
    'rare shiny', 'shiny rare', 'amazing rare', 'radiant rare', 'rare prime',
    'rare break', 'rare holo ex', 'rare holo gx', 'rare holo v', 'rare holo vmax',
    'rare holo vstar', 'rare holo lv x', 'rare holo star', 'legend', 'hyper rare',
    'double rare', 'ace spec rare', 'trainer gallery rare holo', 'shiny ultra rare',
];

/** Rarities that are essentially never foiled in the main set. */
const NEVER_HOLO_RARITY = ['common', 'uncommon'];

/**
 * Work out the printing when the photo could not settle it.
 *
 * Returns the same values as normalizePrinting, plus whether the answer was
 * read off the card or inferred — callers use that to set price confidence
 * rather than presenting a guess as a fact.
 *
 * @returns {{printing: string, inferred: boolean, basis: string}}
 */
export function resolvePrinting({ holo_type, is_holo, is_holographic, rarity } = {}) {
    const direct = normalizePrinting(holo_type, is_holo ?? is_holographic);
    if (direct !== 'unknown') return { printing: direct, inferred: false, basis: 'read from the card' };

    const r = normalizeText(rarity);
    if (r) {
        if (ALWAYS_HOLO_RARITY.some(x => r === x || r.startsWith(x) || r.includes(x))) {
            return { printing: 'holo', inferred: true, basis: `rarity "${rarity}" is only printed holo` };
        }
        if (NEVER_HOLO_RARITY.includes(r)) {
            return { printing: 'normal', inferred: true, basis: `rarity "${rarity}" is not printed holo` };
        }
    }
    return { printing: 'unknown', inferred: true, basis: 'neither the foil pattern nor the rarity settled it' };
}
 ...
 *
 * The set NAME is deliberately not part of it, and that is the whole point.
 *
 * A card's set name is the one field on a scan that nothing prints: the model
 * infers it from a symbol, and it infers inconsistently. The same Meganium
 * numbered 010/132 was read once as "Scarlet & Violet-Temporal Forces" and once
 * as "Paldea Evolved", and because the name was part of this key those became
 * two rows — two cards in the collection, two entries in the count, two prices
 * in the total, for one piece of cardboard. Repeated across a shelf that is how
 * 657 cards present as 1,474.
 *
 * What IS printed is the number, and the number carries its own set
 * discriminator: "010/132" says this came from a set of 132 cards. Two scans of
 * one card always agree on that, however they disagree about what the set is
 * called. So identity rests on the denominator, and falls back to the printed
 * set code — also on the card — for promos and subsets that have no denominator.
 *
 * This is the same rule the matcher already follows: what is printed on the card
 * decides which printing it is, and what the model guessed can never override it.
 */
export function buildVariantKey(card) {
    const { number, printedTotal } = parseCardNumber(card.card_number);
    // Prefer the denominator; fall back to the printed set code; failing both,
    // an empty slot, which groups a Pokémon's undenominated promos together
    // rather than splitting them on a guess.
    const setEvidence = printedTotal
        ? `of${printedTotal}`
        : normalizeText(card.set_code || '');
...
/**
 * The variant key a card will have *once stored*.
 *
 * Storage fills two blanks on the way in — `holo_type` becomes the string
 * "Unknown" and `is_holo` becomes false — and `buildVariantKey` reads both. A
 * card with neither field set therefore keys as "unknown" before the insert and
 * "normal" after it, so looking a card up by the key of the object in your hand
 * could never find the row just written from that same object. Every card whose
 * foil nobody had established — which is most of them, since the foil is the one
 * thing never to read off a photograph — silently failed to deduplicate:
 * re-running an extraction, or re-adding a list, appended a second row for the
 * same piece of cardboard rather than recognising it.
 *
 * Both sides ask this, so both sides get the same answer. It deliberately does
 * not change what gets stored: existing keys stay exactly as they are, and
 * lookups simply start agreeing with them.
 */
export function storedVariantKey(card) {
    return buildVariantKey({
        ...card,
        holo_type: card?.holo_type || 'Unknown',
        is_holo: (card?.is_holographic || card?.is_holo) ? 1 : 0,
    });
}
