...
import { normalizeText, normalizeCardNumber, normalizePrinting, isTruthy } from './identity.js';
...
export function pickTcgplayerVariantPrice(prices, { printing, isFirstEdition } = {}) {
    ...
    for (const key of tcgplayerVariantPreference(printing, isFirstEdition)) {
        const price = priceFromVariantNode(prices[key]);
        if (price) return { price, variant: key, variantMatched: true };
    }

    let cheapest = null;
    for (const [key, node] of Object.entries(prices)) {
        const price = priceFromVariantNode(node);
        if (price && (!cheapest || price < cheapest.price)) {
            cheapest = { price, variant: key, variantMatched: false };
        }
    }
    return cheapest;
}
 ...
 */
export async function aggregateQuotes(quotes, { axios }) {
    ...
    const tcgplayerQuotes = normalised.filter(q => q.marketplace === 'tcgplayer');
    const marketplace = tcgplayerQuotes.length ? 'tcgplayer' : 'cardmarket';
    let pool = tcgplayerQuotes.length ? tcgplayerQuotes : normalised;
    ...
    if (marketplace === 'tcgplayer') confidence += 0.1;
...
export function priceContextFor(card) {
    return {
        printing: normalizePrinting(card?.holo_type, card?.is_holo ?? card?.is_holographic),
