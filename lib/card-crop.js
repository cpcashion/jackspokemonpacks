...
/**
 * How far to turn a cut-out card so it stands upright.
 *
 * A card cut out of a photo keeps whatever angle it was lying at, and cards
 * laid on a table are routinely on their side. Sixteen crops at four different
 * angles is what makes a collection read as a pile of snapshots rather than a
 * set — and a card on its side is not merely untidy, it is unreadable at grid
 * size, which is the whole complaint.
 *
 * The model is asked how the card sits, because it can see where the name is
 * and geometry cannot. Where it did not answer, the shape is still worth
 * acting on: a Pokemon card is taller than it is wide, so a landscape crop is
 * certainly lying on its side even though nothing here can tell which side. A
 * quarter turn makes it upright-or-upside-down instead of sideways, which is
 * the difference between a card you can read and one you cannot.
 *
 * @param {number|undefined} reported  degrees clockwise the model reported
 * @param {{width:number, height:number}} [size]  the cut-out region
 * @returns {{degrees:number, certain:boolean}}
 */
export function uprightRotation(reported, size) {
    const r = Number(reported);
    if (Number.isFinite(r)) {
        // Accept only the four square turns; anything else is a misread, and
        // rotating a card by 37 degrees would letterbox it into a mess.
        const normalized = ((Math.round(r / 90) * 90) % 360 + 360) % 360;
        if (r >= -360 && r <= 360) return { degrees: normalized, certain: true };
    }
    const { width, height } = size || {};
    if (width > 0 && height > 0 && width > height) return { degrees: 90, certain: false };
    return { degrees: 0, certain: true };
}

/**
 * Is this stored picture a card lying on its side?
 *
 * Turning new crops upright does nothing for the ones already saved, and those
 * are the whole collection. Straightening them means deciding, from the stored
 * image alone, whether it is a card on its side — and getting that wrong turns
 * an upright card sideways, which is worse than leaving it.
 *
 * A crop cut to a card's own edges has the card's proportions, so one lying
 * down is about 88/63 = 1.40 wide for tall. A photograph framed in landscape
 * with an upright card in the middle is far wider — 16/9 is 1.78 — and its card
 * is not sideways at all. The window between them is what makes this safe: wide
 * enough to catch a tight crop however loosely it was cut, narrow enough to
 * leave a phone's landscape frame alone.
 *
 * A picture of several cards is legitimately landscape and is never a card, so
 * callers must exclude source photos before asking.
 */
export const SIDEWAYS_MIN_RATIO = 1.15;
export const SIDEWAYS_MAX_RATIO = 1.75;

export function looksSideways({ width, height } = {}) {
    if (!(width > 0) || !(height > 0)) return false;
    const ratio = width / height;
    return ratio >= SIDEWAYS_MIN_RATIO && ratio <= SIDEWAYS_MAX_RATIO;
}
