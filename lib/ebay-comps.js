 ...
 */
export function titleMatchesCard(title, { name, number, printedTotal, setName, requireNumber = true } = {}) {
    ...
    const setHit = cleanSet.length >= 3 && t.replace(/[^a-z0-9]+/g, ' ').includes(cleanSet);
    ...
    return {
        ...
        confident: nameHit && (numberHit || (!requireNumber && setHit)),
    };
}
